...
function parseReceiptByRules(text, fileName = "") {
  ...
  const totalResult = findTotalAmountResult(lines);
  const amount = totalResult.amount;
  ...
  return {
    ...
    _amount_confidence: totalResult.confidence,
    _amount_source: totalResult.source
  };
}
